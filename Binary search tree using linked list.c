  //C program to Implement a Binary Search Tree using Linked List and perform traversal operations.
    ...
    int main()
    {
        ...
        root = new_node('A'); // making a new node

        root->left_child = new_node('B'); //left child of root

        root->right_child = new_node('F'); //right child of root

        root->left_child->left_child = new_node('D'); // new node

        root->left_child->right_child = new_node('C'); // new node
